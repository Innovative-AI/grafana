...
import (
	...
	"github.com/grafana/grafana/pkg/services/ngalert/util"
...
func TestAlertingTicker(t *testing.T) {
	_, store := util.SetupTestEnv(t, 1)
	t.Cleanup(registry.ClearOverrides)

	alerts := make([]*models.AlertDefinition, 0)
	// create alert definition with zero interval (should never run)
	alerts = append(alerts, util.CreateTestAlertDefinition(t, store, 0))

	// create alert definition with one second interval
	alerts = append(alerts, util.CreateTestAlertDefinition(t, store, 1))
	...
	err := store.UpdateAlertDefinition(&models.UpdateAlertDefinitionCommand{
		UID:             alerts[0].UID,
	...
	err = store.DeleteAlertDefinitionByUID(&models.DeleteAlertDefinitionByUIDCommand{UID: alerts[1].UID, OrgID: alerts[1].OrgID})
	require.NoError(t, err)
	...
	err = store.UpdateAlertDefinitionPaused(&models.UpdateAlertDefinitionPausedCommand{UIDs: []string{alerts[2].UID}, OrgID: alerts[2].OrgID, Paused: true})
	require.NoError(t, err)
	...
	err = store.UpdateAlertDefinitionPaused(&models.UpdateAlertDefinitionPausedCommand{UIDs: []string{alerts[2].UID}, OrgID: alerts[2].OrgID, Paused: false})
	require.NoError(t, err)
